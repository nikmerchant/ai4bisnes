...
import type { MarketingPlanInputs, PlanArtifact, PlanItem, SavedPlan } from "../plan-engine/types";

const DRAFT_KEY = "ai4bisnes-marketing-plan-draft-v1";
const CHANNELS = ["Facebook", "Instagram", "TikTok", "WhatsApp", "Email", "Kedai fizikal"];
...
async function post(body: Record<string, unknown>) {
  const res = await fetch("/app/marketing-plan/api", {
    method: "POST",
...
export function MarketingPlanClient({ savedPlans }: { savedPlans: SavedPlan[] }) {
  ...
  const [inputs, setInputs] = useState<MarketingPlanInputs>({ startDate: new Date().toISOString().slice(0, 10), channels: ["WhatsApp"], objective: "Naikkan jualan", promotion: "", intensity: "Sederhana" });
  ...
  function toggleChannel(channel: string) {
    const channels = inputs.channels.includes(channel) ? inputs.channels.filter((item) => item !== channel) : [...inputs.channels, channel];
    setInputs({ ...inputs, channels });
  }
  ...
  async function importResponse() {
    ...
    try {
      ...
      setMessage(`${data.artifact.items.length} tindakan berjaya disusun. Semak sebelum simpan perubahan.`);
    } catch (err) { setMessage(err instanceof Error ? err.message : "Ralat berlaku."); }
    setLoading(false);
  }
  ...
  async function saveChanges() {
    ...
    try {
      ...
      setMessage("✓ Pelan pemasaran telah disimpan.");
      localStorage.removeItem(DRAFT_KEY);
    } catch (err) { setMessage(err instanceof Error ? err.message : "Ralat berlaku."); }
    setLoading(false);
  }
    ...
    <div className="grid gap-6">
      {!prompt && !artifact && savedPlans.length > 0 && (
        <section className="grid gap-3 rounded-2xl border border-zinc-200 p-5 dark:border-zinc-800">
          <h2 className="font-bold">Pelan tersimpan</h2>
          {savedPlans.map((plan) => (
          ...
          <p className="text-xs text-neutral-500">Atau cipta pelan baharu menggunakan borang di bawah.</p>
        </section>
      )}
      {!prompt && !artifact && (
        <form onSubmit={preparePrompt} className="grid gap-4 rounded-2xl border border-zinc-200 p-5 dark:border-zinc-800">
          <div><h2 className="font-bold">1. Sediakan pelan</h2><p className="text-xs text-neutral-500">Profil bisnes digunakan secara automatik. Tiada data dihantar ke AI.</p></div>
          ...
          <label className="grid gap-1 text-sm font-medium">Matlamat utama<input className={inputClass} maxLength={120} required value={inputs.objective} onChange={(e) => setInputs({ ...inputs, objective: e.target.value })} /></label>
          <fieldset className="grid gap-2"><legend className="text-sm font-medium">Saluran pemasaran</legend><div className="flex flex-wrap gap-2">{CHANNELS.map((channel) => <button type="button" key={channel} onClick={() => toggleChannel(channel)} className={`rounded-full border px-3 py-1.5 text-xs ${inputs.channels.includes(channel) ? "border-violet-600 bg-violet-600 text-white" : "border-zinc-300 dark:border-zinc-700"}`}>{channel}</button>)}</div></fieldset>
          <label className="grid gap-1 text-sm font-medium">Promosi / kempen semasa (pilihan)<textarea className={inputClass} maxLength={500} rows={3} value={inputs.promotion} onChange={(e) => setInputs({ ...inputs, promotion: e.target.value })} /></label>
          <label className="grid gap-1 text-sm font-medium">Tahap aktiviti<select className={inputClass} value={inputs.intensity} onChange={(e) => setInputs({ ...inputs, intensity: e.target.value as MarketingPlanInputs["intensity"] })}><option>Ringan</option><option>Sederhana</option><option>Agresif</option></select></label>
          <button disabled={loading} className="rounded-full bg-violet-600 py-3 font-bold text-white disabled:opacity-50">{loading ? "Menyediakan..." : "Sediakan pelan 30 hari"}</button>
        </form>
      )}
      ...
      {artifact && (
        <section className="grid gap-4">
          ...
          <button disabled={loading} onClick={saveChanges} className="sticky bottom-4 rounded-full bg-violet-600 py-3 font-bold text-white shadow-lg disabled:opacity-50">{loading ? "Menyimpan..." : "Simpan pelan"}</button>
          <button type="button" onClick={startNew} className="text-sm text-neutral-500 underline">Cipta pelan baharu</button>
        </section>
      )}
