export type VisualPackagingAccessUser = { id: string; email?: string | null };
export type VisualPackagingAccessInput = { nodeEnv?: string; deploymentTarget?: string; enabled?: string; allowlist?: string; user: VisualPackagingAccessUser };

function enabled(value: string | undefined) { return value?.trim().toLowerCase() === "true"; }
function allowlisted(value: string | undefined, user: VisualPackagingAccessUser) {
...
export function resolveVisualPackagingAccess(input: VisualPackagingAccessInput) {
