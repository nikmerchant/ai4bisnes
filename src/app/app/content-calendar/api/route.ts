...
import { buildContentCalendarPrompt } from "../../plan-engine/prompt";
import { parsePlanResponse, validateArtifactForUpdate, validateCalendarInputs } from "../../plan-engine/validation";
...
export async function POST(req: NextRequest) {
  ...
  if (ctx.profile.tier !== "max") return error("Content Calendar tersedia untuk pelan MAX.", 403);
  ...
  try {
    ...
    if (body.action === "prompt") {
      const inputs = validateCalendarInputs(body.inputs);
      const prompt = buildContentCalendarPrompt(inputs, ctx.profile);
      const { data, error: insertError } = await admin
        .from("generated_outputs")
        .insert({
          user_id: ctx.user.id,
          task_slug: "content-calendar-draft",
          task_title: "Kalendar Kandungan",
    ...
    if (body.action === "update") {
      ...
      const artifact = validateArtifactForUpdate(body.artifact);
      const { data: existing } = await admin
        .from("generated_outputs")
        .select("id, inputs")
        .eq("id", outputId)
