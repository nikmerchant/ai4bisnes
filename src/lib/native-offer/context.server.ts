...
export type NativeOfferProfileRow = {
...
function categoryName(value: NativeOfferProfileRow["categories"]) {
  if (Array.isArray(value)) return value[0]?.name_ms || "";
  return value?.name_ms || "";
}

export async function loadNativeOfferContext() {
  ...
  const profile = data as NativeOfferProfileRow;
  if (!profile.onboarded) return { ok: false as const, reason: "not_onboarded" as const };
  const businessProfile: NativeOfferBusinessProfile = {
  ...
  return {
    ...
    business: buildOfferBusinessContextSnapshot(businessProfile),
  };
}
