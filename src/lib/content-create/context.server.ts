...
import { buildContentCreateBusinessContextSnapshot } from "./domain";
...
function category(value: Profile["categories"]) {
  return Array.isArray(value) ? value[0]?.name_ms || "" : value?.name_ms || "";
}

export async function loadContentCreateContext() {
  ...
  return {
    ...
    business: buildContentCreateBusinessContextSnapshot({
      businessName: profile.business_name || "Bisnes saya",
      category: category(profile.categories),
